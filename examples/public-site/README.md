...
```bash
npm run build
node packages/cli/dist/index.js plan examples/public-site/scenario.yml
```

For machine-readable output:

```bash
node packages/cli/dist/index.js plan examples/public-site/scenario.yml --json
```
...
```bash
node packages/cli/dist/index.js run examples/public-site/scenario.yml --output aee-output/public-site
```
...
The current safe comparison hovers the public **GitHub** navigation link on this project's own demo site in one fresh context and focuses the same link in another. It compares only the declared URL, visibility, and text outcome. It does not activate the link, sign in, create an account, or enter a payment workflow. Every action gets fresh viewport and full-page images, DOM, accessibility-tree, focus, and Axe evidence, and the combined result is stored in `interaction-trace.json`.
...
Some sites route visitors through homepage variants for experiments. The runner first obtains one initial storage state and gives an identical copy to both fresh lanes, preventing experiment assignment from masquerading as an input-method difference. The final URLs are still compared between lanes. Observed text is whitespace-normalized before exact comparison while the raw DOM remains in the evidence artifacts.

This is still a bounded execution of the declared scenario, not a complete accessibility assessment of the site or a WCAG conformance claim.
