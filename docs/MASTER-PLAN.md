...
- 2026-09-26 — Registry patterns live in two places: `patterns` on each entry (what explains the concept) and `pattern` on each axe detection requirement (the one page a finding links to); a test keeps the second inside the first. Why: a `button-name` finding must link to buttons, not to links or forms.
