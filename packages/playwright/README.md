...
`runInputComparison(...)` and `runVirtualScreenReaderLane(...)` automatically write `manifest.json`. The manifest itself is not self-indexed because a file cannot contain a stable checksum of its own final bytes.
